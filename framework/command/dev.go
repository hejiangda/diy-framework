...
import (
	...
	app2 "github.com/hejiangda/diy-framework/framework/provider/app"
...
func (p *Proxy) rebuildBackend() error {
	// 重新编译hade
	cmdBuild := exec.Command("./diy-framework", "build", "backend")
...
func (p *Proxy) restartBackend() error {
	...
	// 使用命令行启动后端进程
	cmd := exec.Command("./diy-framework", "app", "start", "--address="+hadeAddress)
...
func (p *Proxy) restartFrontend() error {
	...
	app := p.c.MustMake(contract.AppKey).(*app2.HadeApp)
